...
// static uint8_t __g_font_buf[1007];//如bin文件存在SPI FLASH可使用此buff

static uint8_t *__user_font_getdata(int offset, int size) {
    //如字模保存在SPI FLASH, SPIFLASH_Read(__g_font_buf,offset,size);
    //如字模已加载到SDRAM,直接返回偏移地址即可如:return
    //(uint8_t*)(sdram_fontddr+offset);
    static uint8_t *font2_buffer = (uint8_t *)0xD0634000;
    return (uint8_t *)font2_buffer + offset;
}
...
static const uint8_t *__user_font_get_bitmap(const lv_font_t *font,
    ...
    if (p_pos[0] != 0) {
        ...
        return __user_font_getdata(pos + sizeof(glyph_dsc_t),
                                   gdsc->box_w * gdsc->box_h * __g_xbf_hd.bpp /
                                       8);
    }
    return NULL;
}
...
// Noto Sans CJK SC Medium,Regular,18
//字模高度：53
// XBF字体,外部bin文件
lv_font_t f_NotoSansCJK_36 = {
